...
// =============================================
// Neural Background — auto-inject on every page
// =============================================
(function() {
    function injectNeuralBg() {
        // Skip if already has a canvas background
        if (document.querySelector('canvas#neural-canvas-global')) return;
        const canvas = document.createElement('canvas');
        canvas.id = 'neural-canvas-global';
            ...
            for (let i = 0; i < nodes.length; i++) {
                for (let j = i + 1; j < nodes.length; j++) {
                    ...
                    if (dist < 160) {
                        ...
                        ctx.strokeStyle = 'rgba(6,182,212,' + (0.18 * (1 - dist / 160)) + ')';
                        ctx.lineWidth = 0.5;
                        ctx.stroke();
                    }
                }
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', injectNeuralBg);
    } else {
        injectNeuralBg();
    }
})();
